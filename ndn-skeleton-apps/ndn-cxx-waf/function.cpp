...
public:
  ...
  void run() {
    ...
    m_face->setInterestFilter(m_funcName, bind(&Func::onInterest, this, _1, _2),
  ...
  void setFuncName(ndn::Name funcName) { m_funcName = funcName; }

private:
  void onInterest(const ndn::InterestFilter &filter,
                  const ndn::Interest &interest) {
    std::cout << "<< Interest: " << interest.getName().get(-1).toSegment()
              << std::endl;
    std::cout << "--------------------------------------------" << std::endl;
    ...
    if (segment > 0) {
      ...
      m_face->expressInterest(interest, bind(&Func::onData, this, _1, _2),
  ...
  void onData(const ndn::Interest &interest, const ndn::Data &data) {
    ...
    std::cout << "Segment No.: " << data.getName().get(-1).toSegment()
              << std::endl;
    std::cout << "Final Block No.: " << m_finalBlockNumber << std::endl;

    // Add Segments to Buffer
    m_receiveBuffer[data.getName().get(-1).toSegment()] =
  ...
  void addToBuffer(const ndn::Data &data) {
    const ndn::Block content = data.getContent();
    m_contentBuffer.insert(m_contentBuffer.end(), &content.value()[0],
  ...
  void createFile(std::string outputFilename) {
    std::cout << "---------------------------------------------------"
              << std::endl;
  ...
  /************************DATA
   * SEGMENTATION****************************************/
  void dataSegmentation(ndn::Name suffix, std::string loadFilename,
                        ndn::Function executedfunction) {
    const uint8_t *buffer;
    size_t bufferSize;
    std::tie(buffer, bufferSize) = loadFile(loadFilename);
    std::cout << "new bufferSize: " << bufferSize << std::endl;
    uint64_t tmp_finalBlockNumber =
        ndn::Producer::getFinalBlockIdFromBufferSize(m_prefix.append(suffix),
                                                     m_funcName, bufferSize);
  ...
  void onTimeout(const ndn::Interest &interest) {
    std::cout << "Timeout Seg No.: " << interest.getName().get(-1).toSegment()
              << std::endl;
    std::cout << "---------------------------------------------" << std::endl;
  }
  ...
  ndn::Name getPrefix(ndn::Name name) {
    std::string prefix;
  ...
  ndn::Name getFilename(ndn::Name name) {
...
private:
  ...
  std::map<uint64_t, std::shared_ptr<ndn::Data>>
      m_dataBuffer; // Buffer for when file is larger
};
