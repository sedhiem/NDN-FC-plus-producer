...
public:
  CallbackContainer()
      : m_interestCounter(0), m_dataCounter(0), m_finalBlockId(0) {}

  void processInfoInterest(Producer &infoProducer, const Interest &interest) {
    // Name filename("test.png");
    const uint8_t *m_buffer;
    size_t m_bufferSize;

    std::string filenameString =
        interest.getName().get(-1).toUri(); // get Filename
    Name fileName(filenameString);
    std::tie(m_buffer, m_bufferSize) = loadFile(filenameString);
    int finalBlockId = Producer::getFinalBlockIdFromBufferSize(
        m_contentPrefix.append(fileName), Name("/"), m_bufferSize);
    ...
    infoProducer.produce(
        fileName, reinterpret_cast<const uint8_t *>(str.c_str()), str.size());

    return;
  }

  void leavingInfoData(Producer &pilotProducer, const Data &data) {
    std::cout << "Leaving Data: " << data.getName() << std::endl;
  }

  void processContentInterest(Producer &contentProducer,
                              const Interest &interest) {
    std::string filenameString =
        interest.getName().get(-2).toUri(); // get FileName
    Name fileName(filenameString);
    ...
    if (m_segmentBuffer.size() == m_finalBlockId + 1) {
      ...
      // std::cout << "Final Block ID: " <<
      // Producer::getFinalBlockIdFromBufferSize(m_contentPrefix.append(Name("test.png")),
      // Name("/"), m_bufferSize) << std::endl;
      std::cout << "SENT PNG FILE" << std::endl;
    }
    return;
  }

  void leavingContentData(Producer &producer, const Data &data) {
    std::cout << "Leaving Data: " << data.getName() << std::endl;
  }
...
int main(int argc, char *argv[]) {
  ...
  Name pilotProducerName("/test/producer");
  Producer pilotProducer(pilotProducerName);
  pilotProducer.setContextOption(
      CACHE_MISS,
      (ProducerInterestCallback)bind(&CallbackContainer::processInfoInterest,
                                     &callback, _1, _2));
  pilotProducer.setContextOption(
      DATA_LEAVE_CNTX,
      (ProducerDataCallback)bind(&CallbackContainer::leavingInfoData, &callback,
                                 _1, _2));
  ...
  contentProducer.setContextOption(
      CACHE_MISS,
      (ProducerInterestCallback)bind(&CallbackContainer::processContentInterest,
                                     &callback, _1, _2));
  contentProducer.setContextOption(
      DATA_LEAVE_CNTX,
      (ProducerDataCallback)bind(&CallbackContainer::leavingContentData,
                                 &callback, _1, _2));
